...
object NimCheck extends Properties("Nim") {
  ...
  val updateWritten = (player: Player, max: Int, state: Int) =>
    for {
      move <- WriterT.liftF[Gen, List[UpdateAndPlay], Move](moveGen(max))
  ...
  val winnerGen: Gen[Winner] = Gen.oneOf(PlayerOne, PlayerTwo)

  def gameAsk: WriterT[Gen, Int, Int] =
    for {
      gameLength <- WriterT.lift[Gen, Int, Int](Gen.posNum[Int])
      _ <- WriterT.tell[Gen, Int](gameLength)
    } yield gameLength

  def gamePlay(stones: Int): WriterT[Gen, Int, Winner] =
    for {
      _ <- WriterT.tell[Gen, Int](stones)
      winner <- WriterT.lift[Gen, Int, Winner](winnerGen)
    } yield winner

  sealed trait Plural[+A]
  case object Zero extends Plural[Nothing]
  case class One[A](a: A) extends Plural[A]
  case object Many extends Plural[Nothing]

  def pluralMonoid[A] = new cats.Monoid[Plural[A]] {
    override def empty: Plural[A] = Zero
    override def combine(a: Plural[A], b: Plural[A]): Plural[A] = (a, b) match {
      case (Zero, a) => a
      case (a, Zero) => a
      case _         => Many
    }
  }
  //property("game should play with the given stones")
